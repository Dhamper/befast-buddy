  ...
  return (
    <AppShell showSession={false}>
      <div className="mx-auto max-w-5xl space-y-6">
        <h1 className="title-light text-4xl sm:text-5xl">Learn more</h1>
        <GlassCard>
          <p className="eyebrow mb-3 text-white/80">What a stroke is</p>
          <p className="text-white/90">
            A stroke happens when blood flow to part of the brain is blocked or a
            vessel bursts. Brain cells in the affected area start dying within
            minutes, so the sooner treatment starts, the more brain is saved.
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {LETTERS.map((l) => (
            <GlassCard key={l.letter} className="flex gap-5">
              <span className="display-xl text-5xl text-white">{l.letter}</span>
              <div>
                <h2 className="eyebrow mb-2 text-white/85">{l.label}</h2>
                <ul className="list-disc space-y-1 pl-5 text-base text-white/90">
                  {l.bullets.map((b) => (
                    <li key={b}>{b}</li>
                  ))}
                </ul>
        ...
        <GlassCard>
          ...
          <p className="text-white/90">
            Clot-dissolving treatment and clot removal are only possible inside a
            limited window from the moment symptoms started. That is why the very
            first thing this prototype asks is when the symptoms began — and why
            it never tells you to wait and see.
          </p>
        </GlassCard>
